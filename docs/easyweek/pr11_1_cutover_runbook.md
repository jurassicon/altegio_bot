...
1. **Первый перенос услуги.** Перед самым первым POST по этой услуге инструмент
   читает полный каталог локации, сверяет цену и длительность с manifest,
   проверяет уникальность характеристик и **записывает ожидание в базу** — до
   POST, в отдельной транзакции. С этого момента ожидание зафиксировано.
...
**Что делать, если услуга изменилась.** Переименование, смена цены или
длительности при том же UUID даёт отказ `service_attributes_changed`, и это
правильно: `plan_digest` при таком изменении не двигается, поэтому старый
reviewed dry-run и старый canary формально всё ещё «подходят» — остановить может
только сохранённое ожидание. Порядок действий:

1. решите, изменение намеренное или нет; если нет — верните услугу в каталоге
   к прежнему виду, и всё продолжит работать без каких-либо действий с базой;
2. если изменение намеренное — это **новый reviewed flow**: обновите manifest под
   новые цену и длительность, снимите свежий dry-run, согласуйте и выполните
   новый canary. Прежние строки ledger при этом не трогаются.

Редактировать таблицу ожиданий руками и чистить ledger не нужно и не следует.
Если по услуге уже есть живые перенесённые записи, а ожидание надо изменить —
это отдельное решение владельца, потому что оно меняет то, чем доказываются
