...
### Как читать исход каждой строки

Отмена — единственная мутация этого шага, и инструмент различает четыре
состояния. Ни одно из них не приводит к автоматическому повторному запросу
отмены.

| `rollback_outcome` | Что произошло | Что делать |
|---|---|---|
| `rolled_back` | PUT отправлен, и отдельный GET показал `is_canceled=true` | ничего |
| `rollback_recovered_from_attempt` | предыдущий запуск отправил PUT, ответа не получил, а запись сейчас читается как отменённая; строка закрыта **без** второго PUT | ничего |
| `rollback_cancel_not_sent` | доказано, что запрос не уходил: чтение перед мутацией не удалось. Право на мутацию возвращено | устранить причину и запустить rollback ещё раз |
| `rollback_refused` | PUT получил детерминированный отказ (401/403/404/422/429). Ничего не изменилось, право на мутацию возвращено | устранить причину и запустить rollback ещё раз |
| `rollback_target_canceled_elsewhere` | запись отменил кто-то другой между проверками и нашим PUT; запрос не отправлялся | проверить вручную, кто отменил |
| `rollback_uncertain` | PUT мог быть принят: timeout, разрыв, 5xx или 2xx без подтверждения | **не повторять вручную**; следующий запуск rollback перечитает запись |
| `rollback_attempt_unresolved` | предыдущая попытка с неизвестным результатом есть, а запись всё ещё активна | решение оператора: проверить запись в EasyWeek и при необходимости отменить вручную |
| `rollback_claim_not_acquired` | правом на отмену этой строки владеет другой запуск | взять `rollback_claim_owner_run_id` из этой же строки отчёта и искать результат в отчёте того запуска; если поля нет, владелец не подтверждён — строка изменилась или исчезла, проверить ledger вручную |
| `rollback_target_modified_after_migration` | время, мастер, услуга, клиент, филиал, длительность или marker изменились — либо запись отменена без нашей попытки | ручная проверка; инструмент такую отмену себе не приписывает |

Что это означает на практике:

- **Ошибка до отправки PUT — это не неизвестная мутация.** Право на мутацию
  возвращается, и более поздняя ручная отмена не будет засчитана как
  восстановление нашей попытки.
- **Детерминированный отказ не даёт права на recovery.** После устранения
  причины следующий явный запуск делает ровно одну новую попытку.
- **Timeout, разрыв соединения и неподтверждённый 2xx блокируют автоматический
  повтор.** Единственный путь дальше — перечитать запись: отменённая закрывает
  строку без запроса, активная остаётся `rollback_attempt_unresolved`.
- **У одной строки одновременно только один владелец мутации.** Два
  параллельных rollback-запуска не отправят две отмены: право берётся одним
  условным UPDATE, и проигравший не делает запросов.
