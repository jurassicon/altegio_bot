...
чтения; скрытые, исторические и удалённые услуги им не покрыты; два чтения — это
наблюдение, а не блокировка каталога. Каталог перечитывается **каждым** запуском;
сохранённый зелёный аудит доказательством не считается. Любая неоднозначность,
неполная страница, изменившаяся цена или пересозданная услуга с новым UUID
останавливают строку.
...
- В отчёте canary после успеха обязаны быть: `target_proven`, сохранённый marker
  ...
  длительность, и `service_evidence.method: catalog_attributes_unique_v1`.
