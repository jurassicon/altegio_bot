...
| `null` на точном `booking-canceled` | «поле не передано»: `service_id=None`, ключ **не входит** в `present_fields`, известная identity сохраняется |
| `null` на `booking-created` / `booking-updated` / `booking-rescheduled` | **детерминированный отказ** |
| boolean, строка, `0`, отрицательное, дробное — на **любом** триггере | **детерминированный отказ** |
...
Почему fail-closed, а не догадка: молча сохранить старую identity означало бы
привязать НОВЫЕ title/amount/цену к СТАРОМУ `service_id`; удалить snapshot —
уничтожить доказанные данные. Отказ оставляет строки нетронутыми и делает
payload видимым оператору.

**Поправка ревизии 43 (2026-09-21).** Прежняя редакция этого раздела
утверждала, что ни одна захваченная доставка не присылала `service_id: null`.
Production event 360 — реальная, необрезанная `booking-canceled` доставка —
это опровергла. Для **этого одного триггера** null теперь доказан и означает
«identity не передана»; отмена не несёт нового выбора услуги, поэтому
привязывать нечего и сохранение доказанной `RecordService` identity безопасно.
Для остальных триггеров смысл null по-прежнему не доказан, и отказ остаётся.
Полный контракт и operator-only recovery уже упавших событий — §40
`INTEGRATION_PLAN.md` и `failed_cancellation_recovery_runbook.md`.
