...
- `--include-neutral` — разрешить копирование нейтральных шаблонов; обязательно
  указать точный список через `--template-name`. Поддерживается только источник
  Karlsruhe: `--source-location ka` (также значение по умолчанию). Это не
  разрешение копировать все маркетинговые шаблоны аккаунта.

Текущие source-маркеры адреса и карты описывают только Karlsruhe. Футер другого
филиала может ошибочно выглядеть нейтральным, поэтому с `--include-neutral`
любой другой источник отклоняется до чтения credentials и обращения к Meta —
в dry-run и apply, в том числе с `--yes`. Без этой опции существующие проверки
источника не меняются. Распознавание произвольных адресов не поддерживается.
