...
Ничего больше делать не надо: производитель может создавать retry-джобы сколько
угодно, отправлять их некому. Самый надёжный и самый простой вариант.
...
Когда причина устранена — просто поднимите outbox:
...
3. Ещё раз почистить EasyWeek-очередь: между шагами 1 и 2 воркер работал со
   старым значением и мог успеть создать новые джобы. Повторите оба `UPDATE` из
   шага 3.

4. Теперь поднять общий outbox — Altegio возобновляется:
...
6. **Проверка после отката.** Убедитесь, что новых EasyWeek-джобов не
   появилось. Подставьте момент остановки outbox (шаг 1) вместо
   `<момент остановки>`:
...
Ожидается пустой результат. Непустой означает, что производитель всё ещё жив —
скорее всего, пропущен или не подействовал шаг 5.2 (пересоздание
`altegio-whatsapp-inbox-worker`). Джобы с `dedupe_key` вида
`delivery_retry:<id>:<n>` — это именно delivery-retry, а не планировщик.

Повторяйте проверку несколько раз в течение получаса после отката:
запоздавшие callback'и приходят не мгновенно.
