...
Закрытие волны durable: в той же транзакции пишется строка
`easyweek_migration_wave_closure` на **каждую** пару company/run, которую snapshot
объявляет закрытой. Row-level marker существует только там, где есть
`status=created` строка, поэтому пара без созданных строк (пустая или только
`failed`) не могла нести доказательство закрытия: после освобождения advisory
lock проверка отвечала «нет», и поздний retry с этим run_id создавал бронирование
в уже закрытой волне. Advisory lock только сериализует; пережить транзакцию может
лишь закоммиченная строка. Откат не оставляет закрытия, повтор того же snapshot
идемпотентен, чужой plan digest даёт `migration_wave_closure_conflict`. Волны,
закрытые до этой ревизии, распознаются по прежнему row-level marker.
...
— `migration_wave_unresolved`; сначала выполняется reconcile. С этой ревизии то
