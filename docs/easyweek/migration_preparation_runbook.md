...
`price_to_pay`, телефон клиента и причина блокировки, если она есть. При явном
staff-scoped разрешении `normalize_duration_to_catalog_for_staff_ids` план
дополнительно показывает исходную и целевую каталожную длительность и факт
нормализации. Это тот
...
## 6a. Приватная таблица для ручной сверки и корректировки

После `reconcile --final` запустите
`altegio_bot.scripts.easyweek_migration_operator_export` через тот же
`easyweek-booking-migration` container, заменив его entrypoint. Команда повторно
читает только будущие активные записи одного явно выбранного мастера в Altegio и
соединяет их с durable migration ledger. CRM она не изменяет.

На выходе создаются два owner-only файла: `irina-bookings.csv` для Excel/Google
Sheets и `irina-bookings.html` с кликабельной колонкой `Open booking`. Каждая
строка содержит Altegio record id, имя, телефон, локальные дату и время,
название услуги, исходную и стандартную EasyWeek-длительность, цену, migration
status, target UUID и прямую ссылку
`https://my.easyweek.io/bookings/uuid/<uuid>`. Ссылка выводится только для
ledger-строки со строгим `status=created` и canonical UUID. Поэтому
`not_migrated`, `blocked`, `uncertain` и остальные незавершённые записи остаются
видны, но не получают недоказанную ссылку.

Таблица содержит PII. Оба файла имеют mode `0600`, каталог — `0700`; их нельзя
коммитить, вставлять в issue/PR или публиковать вместе с обычным PII-free
migration report. CLI печатает только количество строк, количество доказанных
...
**Если plan показывает `local_target_mismatch`** — локальная запись активна, а
живой EasyWeek говорит, что бронирование отменено — handover прав, что
отказывается, и одна такая строка блокирует всю атомарную волну. Подтверждённая
причина такого расхождения: `booking-canceled` доставка с `service_id: null`,
упавшая как `invalid_payload` (§40, production event 360). Её нельзя починить
ни SQL-правкой, ни повторным handover: сначала выполняется отдельная
операторская процедура
[`failed_cancellation_recovery_runbook.md`](failed_cancellation_recovery_runbook.md),
и только после её `verify` этот plan запускается заново. Recovery не трогает
Altegio-напоминания и не ставит ownership marker — это по-прежнему работа
handover.
...
- Не утверждает, что EasyWeek API принимает индивидуальную длительность или
  цену. По умолчанию запись, растянутая руками, попадает в
  `manual_adjustment_candidate`. Явный staff-scoped opt-in в manifest может
  вместо этого намеренно создать её со стандартной каталожной длительностью;
  индивидуальная цена всё равно остаётся ручной.
