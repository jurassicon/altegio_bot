...
> ОБЛАСТЬ ДЕЙСТВИЯ. Гарантия покрывает именно эти пути и закреплена тестами
> (`test_worker_log_hygiene.py`, включая статическую проверку всех logger-вызовов
> в `whatsapp_inbox_worker`). Остальные модули — `outbox_worker`,
> `promo_lead_handler`, `chatwoot_client`, провайдеры, campaign-код — **всё ещё
> пишут телефон** в свои логи. Это известный незакрытый участок, а не гарантия;
> при разборе инцидентов и при выдаче доступа к логам исходите из того, что
> телефон в них встречается. Приведение остальных модулей к той же политике —
> отдельная задача.
