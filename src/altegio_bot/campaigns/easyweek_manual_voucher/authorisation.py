"""What turns an operator's "yes" into permission for exactly one stage (§37.2).
...
Three properties, each earned the hard way in §35 and §36:

*Per stage.* One plan cannot cover the canary. The moment ``create`` succeeds, a
...
Separate from §36's, deliberately
---------------------------------
The scope string is inside the digest and the canary name is inside the
confirmation phrase, so a §36 approval cannot authorise a §37.2 stage even if an
operator pastes it — and the phrase an operator types says out loud which canary
    ...
    return _digest_over(
        {
            ...
            "schema_version": MANUAL_VOUCHER_SCHEMA_VERSION,
            "stage": stage,
            "snapshot": snapshot,
...
class StagePlan:
    ...
    def phrase_for(self, digest: str) -> str:
        # Names the canary, not only the stage: an operator typing this should
        # be able to see from the phrase alone which one they are authorising.
        return f"{self.stage}-manual-voucher-{digest[:12]}"
    ...
    def as_safe_dict(self) -> dict[str, Any]:
        return {
            "mode": "manual_voucher_stage_plan",
            "canary_scope": MANUAL_VOUCHER_SCOPE,
            "schema_version": MANUAL_VOUCHER_SCHEMA_VERSION,
            "stage": self.stage,
            "ready": self.ready,
