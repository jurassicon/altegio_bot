...
async def verify_reminder_is_current(
    *,
    ...
    snapshot, _snapshot_error = multi_service_snapshot_for_job(
        record_raw=getattr(record, "raw", None),
        job_payload=getattr(job, "payload", None),
    )
    claims_pair = multi_service_job_digest(getattr(job, "payload", None)) is not None
    if snapshot is not None or claims_pair:
