...
class ChatwootClientFixed:
    ...
    async def find_contact_by_phone(self, phone_e164: str) -> dict | None:
        ...
        response = await self._request_with_retry("GET", url, headers=self._headers(), params=params)
    ...
    async def create_contact(
        self, phone_e164: str, name: str | None = None, custom_attributes: dict | None = None
    ) -> dict:
        """Create new contact."""
        ...
        response = await self._request_with_retry("POST", url, headers=self._headers(), json=payload)

        data = response.json()
        return data.get("payload", {}).get("contact", {})

    async def update_contact_attributes(self, contact_id: int, custom_attributes: dict) -> dict:
        """Update contact custom attributes."""
        url = f"{self.base_url}/api/v1/accounts/{self.account_id}/contacts/{contact_id}"

        payload = {"custom_attributes": custom_attributes}

        response = await self._request_with_retry("PUT", url, headers=self._headers(), json=payload)
...
async def get_clients_with_recent_records(months: int = 12, limit: int | None = None) -> list[Client]:
...
async def get_last_appointment_date(client_id: int) -> str | None:
    ...
    async with SessionLocal() as session:
        stmt = select(Record.starts_at).where(Record.client_id == client_id).order_by(Record.starts_at.desc()).limit(1)
        result = await session.execute(stmt)
...
async def migrate_one_client(client: Client, chatwoot: ChatwootClientFixed, dry_run: bool = False) -> tuple[str, str]:
    ...
    try:
        ...
        if existing:
            await chatwoot.update_contact_attributes(existing["id"], custom_attrs)
            return ("updated", f"Updated contact id={existing['id']}")
        else:
            contact = await chatwoot.create_contact(
                client.phone_e164, name=client.display_name, custom_attributes=custom_attrs
            )
...
async def migrate_contacts(
    ...
    for i, client in enumerate(clients, 1):
        ...
        log_func(f"[{i}/{len(clients)}] {client.phone_e164} ({client.display_name}): {status} - {message}")
...
async def main() -> None:
    parser = argparse.ArgumentParser(description="Migrate contacts to Chatwoot with retry logic")
    parser.add_argument("--dry-run", action="store_true", help="Preview without changes")
    parser.add_argument("--limit", type=int, help="Max contacts to process")
    parser.add_argument("--months", type=int, default=12, help="Include clients with appointments in last N months")
    parser.add_argument("--delay", type=float, default=0.6, help="Delay between API requests in seconds")
