...
async def _resolve_all_company_ids(session: AsyncSession) -> list[int]:
    res = await session.execute(
        select(Client.company_id).distinct().order_by(Client.company_id)
    )
    ...
    *,
    ...
    stmt = (
        select(
            Client,
            func.coalesce(
                stats_subq.c.total_records, 0
            ).label('total_records'),
            func.coalesce(
                stats_subq.c.arrived_records, 0
            ).label('arrived_records'),
        )
...
def _compute_summary(
    ...
    return {
        ...
        'excluded_opted_out': sum(
            1 for c in candidates if c.excluded_reason == 'opted_out'
        ),
        'excluded_more_than_one_record': sum(
            1 for c in candidates
            if c.excluded_reason == 'more_than_one_record'
        ),
        'excluded_has_arrived': sum(
            1 for c in candidates if c.excluded_reason == 'has_arrived'
        ),
        'excluded_no_phone': sum(
            1 for c in candidates if c.excluded_reason == 'no_phone'
        ),
    }
...
def _format_table(candidates: list[CandidateInfo]) -> str:
    ...
    headers = [
        'company_id', 'altegio_id', 'name', 'phone',
        'records', 'arrived', 'optout', 'eligible', 'reason',
    ]
    rows = [
        [
            str(c.company_id),
...
def _print_candidates(
    ...
    *,
    ...
    if fmt == 'json':
        content = json.dumps(
            [asdict(c) for c in all_candidates], indent=2, default=str
        )
        print(content)
    else:
    ...
    async with SessionLocal() as session:
        async with session.begin():
            run = CampaignRun(
                ...
                excluded_more_than_one_record=summary[
    ...
    *,
    ...
    match = next(
        (
            ...
            if isinstance(t, dict) and t.get('title') == DEFAULT_CARD_TYPE_TITLE
        ),
        None,
    )
...
async def _run_mode_dry_run(
    ...
    *,
    ...
    print(
        f'\n[dry-run] Would send {len(eligible)} message(s) '
        f'via template {template_name!r}.'
    )
...
async def _run_mode_send_test(
    ...
    *,
    ...
    print(
        f'\n[send-test] Eligible candidates: {eligible_count}. '
        f'Sending test message to {test_phone} only.\n'
    )
...
async def _run_mode_send_real(
    ...
    *,
    ...
    try:
        ...
        for candidate in eligible:
            ...
            if cid not in card_type_cache:
                card_type_cache[cid] = await _resolve_card_type_id(
                    loyalty, cid, card_type_id=card_type_id
                )
            ...
            issued_num = str(
                card_resp.get('loyalty_card_number') or card_num
            )
            card_id = str(
    ...
    await _complete_campaign_run(
        run_id, sent=sent, failed=errors
    )
    ...
    print(
        f'\n[send-real] campaign_run_id={run_id} '
...
async def run_monthly_newsletter_smart(
    *,
    ...
    logger.info(
        'Monthly newsletter smart mode=%s period=[%s, %s) '
        'company_id_arg=%r',
        mode,
        period_start.date(),
        period_end.date(),
...
async def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description='Monthly newsletter smart runner (list/dry-run/send-test/send-real).'
    )
