...
def _ack_text(cmd: str) -> str:
    if cmd == 'stop':
        return (
            'Sie haben sich von Marketing-Nachrichten abgemeldet. '
            'Um wieder zu abonnieren, senden Sie START.'
        )

    return (
        'Sie sind wieder angemeldet und erhalten Marketing-Nachrichten. '
        'Um sich abzumelden, senden Sie STOP.'
    )
...
async def handle_event(
    ...
    logger.info(
        'wa_cmd=%s phone=%s sender_phone_number_id=%s sender_id=%s '
        'clients_updated=%s jobs_canceled=%s event_id=%s',
        cmd,
        phone_e164,
        phone_number_id,
        sender_id,
...
async def process_one_event(
    ...
    async with SessionLocal() as session:
        async with session.begin():
            stmt = (
                select(WhatsAppEvent)
                .where(WhatsAppEvent.id == event_id)
                .with_for_update()
            )
            res = await session.execute(stmt)
...
def main() -> None:
    ...
    raise SystemExit(
        'Run as a script: python -m altegio_bot.scripts.run_whatsapp_inbox_worker'
    )
