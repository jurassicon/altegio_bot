...
Covers three scenarios requested in the code review:
1. The backfill script in dry-run mode must not modify any jobs.
2. The backfill script must cancel queued review_3d jobs whose clients have
   more than MAX_VISITS_FOR_REVIEW attended visits.
3. The outbox worker must cancel a review_3d job at runtime when the client
   already has more than MAX_VISITS_FOR_REVIEW attended visits (last line of
   defence against stale / manually inserted jobs).
"""
...
def _make_review_3d_job(attended_over_limit: bool = True) -> FakeJob:
    return FakeJob(
        id=JOB_ID,
