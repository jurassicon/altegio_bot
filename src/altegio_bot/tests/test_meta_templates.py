...
def test_resolve_ra_record_created() -> None:
    assert resolve_meta_template(RA, 'record_created') == (
...
def test_resolve_ra_record_updated_fallback() -> None:
    ...
    assert resolve_meta_template(RA, 'record_updated') == (
        'kitilash_ka_record_updated_v1'
    )
...
def test_params_record_created_ka() -> None:
    ...
    assert params == [
        'Anna', 'Tanja', '10.02.2026', '14:00',
        'Mascara — 60.00€', '60.00', 'https://short.link/abc',
    ]


def test_params_record_created_new_client_ka() -> None:
    params = build_template_params(
        'kitilash_ka_record_created_new_client_v1', _CTX
    )
    assert params == [
        'Anna', 'Tanja', '10.02.2026', '14:00',
        'Mascara — 60.00€', '60.00', 'https://short.link/abc',
...
def test_params_record_canceled() -> None:
    ...
    assert params == [
        'Anna', '10.02.2026', '14:00',
        'Mascara — 60.00€', 'https://booking.link/',
    ]
...
def test_params_reminder_24h() -> None:
    ...
    assert params == [
        'Anna', 'Tanja', '10.02.2026', '14:00',
        'Mascara — 60.00€', 'https://short.link/abc',
    ]
...
def test_params_newsletter() -> None:
    params = build_template_params(
        'kitilash_ka_newsletter_new_clients_monthly_v2', _CTX
    )
    assert params == ['Anna', 'https://booking.link/', '']
...
def test_params_newsletter_with_loyalty_card() -> None:
    ...
    params = build_template_params(
        'kitilash_ka_newsletter_new_clients_monthly_v2', ctx
    )
    assert params == [
        'Anna',
        'https://booking.link/',
...
def test_params_newsletter_v1_legacy() -> None:
    params = build_template_params(
        'kitilash_ka_newsletter_new_clients_monthly_v1', _CTX
    )
    assert params == ['Anna', 'https://booking.link/']
