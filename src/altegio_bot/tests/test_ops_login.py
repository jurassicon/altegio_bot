...
async def test_login_post_valid_sets_cookie_and_redirects(
    ...
    response = await client.post("/ops/login", data={"username": _USER, "password": _PASS})
...
async def test_login_post_invalid_returns_401(client: AsyncClient) -> None:
    response = await client.post("/ops/login", data={"username": _USER, "password": "wrongpass"})
...
async def test_session_cookie_grants_access_to_protected_route(
    ...
    login = await client.post("/ops/login", data={"username": _USER, "password": _PASS})
