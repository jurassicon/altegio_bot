...
async def list_runs(
    ...
    async with SessionLocal() as session:
        ...
        if mode:
            stmt = stmt.where(CampaignRun.mode == mode)

        runs = (await session.execute(stmt)).scalars().all()

        # Фильтр по company_id (company_ids — JSON-массив)
        if company_id is not None:
            runs = [
                r for r in runs
                if company_id in (r.company_ids or [])
            ]
...
async def get_recipients(
    ...
    async with SessionLocal() as session:
        ...
        if excluded_reason:
            stmt = stmt.where(
                CampaignRecipient.excluded_reason == excluded_reason
            )
...
async def get_monthly_dashboard(
    ...
    async with SessionLocal() as session:
        return await monthly_dashboard(
            session, year=year, month=month, company_ids=cids
        )
...
async def plan_followup_endpoint(
    ...
    async with SessionLocal() as session:
        async with session.begin():
            # Применяем override параметров, если переданы
            run = await session.get(CampaignRun, run_id)
            if run is None:
                raise HTTPException(
