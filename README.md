...
3. Altegio sends a booking webhook (create/update record event).
4. `inbox_worker` processes the webhook and calls `try_apply_promo_discount`.
5. The function matches the booking to the client's active `PromoLead` by phone number.
...
8. A `queued` `OutboxMessage` (template `wa_promo_discount_applied`) is created for
